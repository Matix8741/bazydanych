...
--------------------------------------------------------------------------------------------------------------------------------------
exec modyfikowanieRachunku  4 , '2016-12-25' , 'wydatki' , 'transport' , 'bilet' , 'pks' , 'jednosci' , '15' , 'klodzko' , '57-300' , -12 , 'brak'
--------------------------------------------------------------------------------------------------------------------------------------
SELECT Transakcje.ID_Transakcji , Transakcje.Data , TypTransakcji.NazwaTypu , RodzajTransakcji.Nazwa , Artykul.NazwaArtykulu , 
Podmiot.NazwaPodmiotu , Lokalizacja.Ulica ,Lokalizacja.NrBudynku , Lokalizacja.Miasto , Lokalizacja.KodPocztowy , 
Transakcje.Kwota , Transakcje.Uwagi , Budzet.Saldo FROM Budzet 
...
CREATE PROCEDURE usuwanieRachunku @idtransakcji int , @data datetime , @typTransakcji nchar(20) , @rodzajTransakcji nchar(20), @artykuly nchar(20), @podmiot nchar(20) ,@podmiotUlica nchar(50), @podmiotBudynek nchar(20), @podmiotMiasto nchar(20) , @podmiotKod nchar(20) , @kwota smallmoney, @uwagi nchar(200)
AS
BEGIN
	DECLARE @idRodzaj int
	DECLARE @idArtykul int
	DECLARE @idPodmiot int
	DECLARE @idlokalizacja int

	DELETE FROM Budzet WHERE Budzet.Transakcja = @idtransakcji
	UPDATE Budzet SET Saldo = Saldo - @kwota WHERE Data >= @data AND Budzet.Transakcja > @idtransakcji

	DELETE FROM Transakcje WHERE ID_Transakcji = @idtransakcji

	SET @idRodzaj = (SELECT ID_Rodzaj FROM RodzajTransakcji WHERE Nazwa = @rodzajTransakcji)
	IF (SELECT COUNT(*) FROM Transakcje WHERE ID_RodzajTransakcji = @idRodzaj) = 0
	BEGIN
		DELETE FROM RodzajTransakcji WHERE Nazwa = @rodzajTransakcji
	END

	SET @idArtykul = (SELECT ID_Artykulu FROM Artykul WHERE NazwaArtykulu = @artykuly)
	IF (SELECT COUNT(*) FROM Transakcje WHERE ID_Artykul = @idArtykul) = 0
	BEGIN
		DELETE FROM Artykul WHERE NazwaArtykulu = @artykuly 
	END

	SET @idPodmiot = (SELECT ID_Podmiotu FROM Podmiot WHERE NazwaPodmiotu = @podmiot AND ID_Lokalizacja = (SELECT ID_Lokalizacji FROM Lokalizacja WHERE Ulica = @podmiotUlica AND NrBudynku = @podmiotBudynek AND Miasto = @podmiotMiasto AND KodPocztowy = @podmiotKod))
	IF (SELECT COUNT(*) FROM Transakcje WHERE ID_Podmiot = @idPodmiot) = 0
	BEGIN
		DELETE FROM Podmiot WHERE NazwaPodmiotu = @podmiot AND ID_Lokalizacja = (SELECT ID_Lokalizacji FROM Lokalizacja WHERE Ulica = @podmiotUlica AND NrBudynku = @podmiotBudynek AND Miasto = @podmiotMiasto AND KodPocztowy = @podmiotKod)
	END

	SET @idlokalizacja = (SELECT ID_Lokalizacji FROM Lokalizacja WHERE Ulica = @podmiotUlica AND NrBudynku = @podmiotBudynek AND Miasto = @podmiotMiasto AND KodPocztowy = @podmiotKod)
	IF (SELECT COUNT(*) FROM Podmiot WHERE ID_Lokalizacja = @idlokalizacja) = 0
	BEGIN
		DELETE FROM Lokalizacja WHERE ID_Lokalizacji = @idlokalizacja
	END
END
...
SELECT Nazwa FROM RodzajTransakcji INNER JOIN Transakcje ON ID_Rodzaj = ID_RodzajTransakcji GROUP BY Nazwa ORDER BY COUNT (*) DESC
SELECT NazwaArtykulu FROM Artykul INNER JOIN Transakcje ON ID_Artykulu = ID_Artykul GROUP BY NazwaArtykulu ORDER BY COUNT (*) DESC
SELECT NazwaPodmiotu FROM Podmiot INNER JOIN Transakcje ON ID_Podmiot = ID_Podmiotu GROUP BY NazwaPodmiotu ORDER BY COUNT (*) DESC
SELECT Ulica FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja GROUP BY Ulica , NrBudynku , Miasto , KodPocztowy ORDER BY COUNT (*) DESC
SELECT NrBudynku FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja GROUP BY Ulica , NrBudynku , Miasto , KodPocztowy ORDER BY COUNT (*) DESC
SELECT Miasto FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja GROUP BY Ulica , NrBudynku , Miasto , KodPocztowy ORDER BY COUNT (*) DESC
SELECT KodPocztowy FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja GROUP BY Ulica , NrBudynku , Miasto , KodPocztowy ORDER BY COUNT (*) DESC
SELECT Kwota FROM Transakcje GROUP BY Kwota ORDER BY COUNT(*) DESC


SELECT Ulica FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja GROUP BY Ulica ORDER BY COUNT (*) DESC
SELECT NrBudynku FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja GROUP BY NrBudynku ORDER BY COUNT (*) DESC
SELECT Miasto FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja GROUP BY Miasto ORDER BY COUNT (*) DESC
SELECT KodPocztowy FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja GROUP BY KodPocztowy ORDER BY COUNT (*) DESC

--inteligentne dobieranie					
SELECT Nazwa FROM RodzajTransakcji INNER JOIN Transakcje ON ID_Rodzaj = ID_RodzajTransakcji GROUP BY Nazwa ORDER BY COUNT (*) DESC
SELECT NazwaArtykulu FROM Artykul INNER JOIN Transakcje ON ID_Artykulu = ID_Artykul GROUP BY NazwaArtykulu ORDER BY COUNT (*) DESC
SELECT NazwaPodmiotu FROM Podmiot INNER JOIN Transakcje ON ID_Podmiot = ID_Podmiotu GROUP BY NazwaPodmiotu ORDER BY COUNT (*) DESC
SELECT Ulica FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja INNER JOIN Transakcje ON ID_Podmiot = ID_Podmiotu GROUP BY Ulica , NrBudynku , Miasto , KodPocztowy ORDER BY COUNT (*) DESC
SELECT NrBudynku FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja INNER JOIN Transakcje ON ID_Podmiot = ID_Podmiotu GROUP BY Ulica , NrBudynku , Miasto , KodPocztowy ORDER BY COUNT (*) DESC
SELECT Miasto FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja INNER JOIN Transakcje ON ID_Podmiot = ID_Podmiotu GROUP BY Ulica , NrBudynku , Miasto , KodPocztowy ORDER BY COUNT (*) DESC
SELECT KodPocztowy FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja INNER JOIN Transakcje ON ID_Podmiot = ID_Podmiotu GROUP BY Ulica , NrBudynku , Miasto , KodPocztowy ORDER BY COUNT (*) DESC
SELECT Kwota FROM Transakcje GROUP BY Kwota ORDER BY COUNT(*) DESC

/*
(SELECT Ulica , COUNT(*) AS Ilosc FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja	INNER JOIN Transakcje ON ID_Podmiot = ID_Podmiotu WHERE NazwaPodmiotu IN /*ZMIENNA Z COMBOBOX PODMIOT*/ (SELECT 'pks') GROUP BY Ulica , NrBudynku , Miasto , KodPocztowy ORDER BY Ilosc DESC)
UNION 
SELECT Ulica FROM (SELECT Ulica , COUNT(*) AS Ilosc FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja INNER JOIN Transakcje ON ID_Podmiot = ID_Podmiotu GROUP BY Ulica , NrBudynku , Miasto , KodPocztowy ORDER BY Ilosc DESC) AS TAB 
SELECT NrBudynku FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja GROUP BY Ulica , NrBudynku , Miasto , KodPocztowy ORDER BY COUNT (*) DESC
SELECT Miasto FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja GROUP BY Ulica , NrBudynku , Miasto , KodPocztowy ORDER BY COUNT (*) DESC
SELECT KodPocztowy FROM Lokalizacja INNER JOIN Podmiot ON ID_Lokalizacji = ID_Lokalizacja GROUP BY Ulica , NrBudynku , Miasto , KodPocztowy ORDER BY COUNT (*) DESC
*/